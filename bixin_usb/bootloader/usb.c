...
static void check_and_write_chunk(void) {
  ...
  // SHA256_CTX ctx = {0};
  // sha256_Init(&ctx);
  // sha256_Update(&ctx, (const uint8_t *)FW_CHUNK + offset, chunk_pos -
  // offset); if (chunk_pos < 64 * 1024) {
  //   // pad with FF
...
void rx_callback(usbd_device *dev, uint8_t ep) {
  (void)ep;
  ...
  if (dev != NULL) {
    // Add test by 22.11.28
    // if (usbd_ep_read_packet(dev, ENDPOINT_ADDRESS_OUT, packet_buf, 64) !=
    // 64)
    //   return;
  ...
  if (flash_state == STATE_OPEN) {
    if (msg_id == 0x0006) {  // FirmwareErase message (id 6)
      ...
      if (firmware_present_new()) {
        ...
        //              "Install new", "firmware?", NULL, "Never do this
        //              without", "your recovery card!", NULL);
  ...
  if (flash_state == STATE_CHECK) {
    ...
    if (UPDATE_ST == update_mode) {
      ...
      //   layoutDialog(&bmp_icon_warning, NULL, NULL, NULL, "ble
      //   installation",
